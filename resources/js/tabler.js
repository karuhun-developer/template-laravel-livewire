...
(function (factory) {
    ...
    if (elements.length) {
        elements.forEach(function (element) {
            var options = {};
            try {
                options = element.getAttribute("data-countup")
            ...
            for (let i = 0; i < pattern.length; ++i) {
                if (this.blocks) {
                    ...
                    if (bName) {
                        ...
                        const maskedBlock =
                            repeat != null
                                ? new IMask.RepeatBlock(blockOpts)
        ...
        maskEquals(mask) {
            return Array.isArray(mask)
                ? this.compiledMasks.every((m, mi) => {
                      ...
                      return (
                          objectIncludes(m, restOpts) && m.maskEquals(oldMask)
                      );
                  })
            ...
            if (matches.length) {
                ...
                if (matches.length === 1) {
                    matches[0]
                        ...
                        .split("")
                        .forEach((mch) =>
                            d.aggregate(super._appendCharRaw(mch))
                        );
                }
                return d;
            }
        ...
        remove(fromPos, toPos) {
            ...
            for (pos = fromPos; pos >= 0; --pos) {
                const matches = this.enum.filter((e) =>
                    this.matchValue(
                        e,
                        this.value.slice(matchFrom, pos),
                        matchFrom
                    )
                );
            ...
            if (accepted) {
                ...
                if (fixedNum != null) {
                    if (this.autofix) {
                        ...
                        skip ||
                            (skip = oldValue === this._value && !flags.tail);
                    } else {
                        accepted = false;
                    }
                }
